...
describe('loadConfig', () => {
  ...
  test('returns parsed config when file is valid', async () => {
    const dir = await mkTmpDir();
    await fs.writeFile(path.join(dir, 'agent-memory.json'), JSON.stringify({ journal: { enabled: true } }));
    const config = await loadConfig(dir);
  ...
  test('returns empty config when schema validation fails', async () => {
    const dir = await mkTmpDir();
    await fs.writeFile(path.join(dir, 'agent-memory.json'), JSON.stringify({ journal: { enabled: 'yes' } }));
    const config = await loadConfig(dir);
    expect(config).toEqual({});
  });
});
...
describe('journal store', () => {
  ...
  test('read throws for nonexistent id', async () => {
    ...
    expect(store.read('99990101-000000-000')).rejects.toThrow('Journal entry not found');
  });
  ...
  test('read rejects path traversal', async () => {
    ...
    expect(store.read('../../../etc/passwd')).rejects.toThrow('Invalid journal entry ID');
  });
